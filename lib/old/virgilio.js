...
//Firgilio is meant to be very hackable.
//To prevent accidental overwrites, all internal variables end with a `$`-sign.
var Firgilio = function Firgilio(options) {
    if (!(this instanceof Firgilio)) {
        return new Firgilio(options);
    }
    EventEmitter.call(this);
    this.options$ = options = options || {};
    this.util$.validateArg('Firgilio', 'options', options, 'object');
    var logOptions = options.logger || {};

    //FIXed: if level===0 or name==='' they're falsy and will be set
    // logOptions.level = logOptions.level || 10; 
    // logOptions.name = logOptions.name || 'firgilio';
    utils.extend({ level: 10, name: 'firgilio' }, logOptions);
    ...
    this.baseFirgilio$ = this;
    //Set the path of this (the base firgilio) namespace.
    this.path$ = 'firgilio';
...
//Make firgilio an EventEmitter.
require('util').inherits(Firgilio, EventEmitter);
//Make the promise library we use available across the application.
Firgilio.prototype.Promise = Promise; //FIX: Promise is attached at the end of
                                      //the file as well, but as bluebird??
Firgilio.prototype.util$ = util$;
//Put all custom errors directly on the prototype.
util$.extend(Firgilio.prototype, errors);
...
Firgilio.prototype.loadModule$ = function loadModule$(module) {
...
Firgilio.prototype.defineAction$ = function defineAction$(path, handler) {
    ...
    //Create a new namespace used as a context for the action itself. FIX: why
    //create a basenamespace first? Why not create a actionNamespace
    //straignt away?
...
firgilio.prototype._createAction$ = function _createAction$(name, handler) {
    //Create a new namespace used as a context for the action itself.
    var actionNamespace = this._createNamespace$(name); //FIX this adds a
    //namespace that gets
    //overridden at this[name] = name
...
// var v = new Virgilio();
// v.namespace$();
// var ns = Virgilio({myoptions:1});

// var a = ns.namespace$('a');
// console.log(a, a.util$);
// a.test = 'test';
// // console.log(ns.namespace$('b'));
// var c = ns.namespace$('c');
// console.log(ns);
// console.log(c);
// console.log(ns.namespace$('a.b').baseVirgilio$);

// // console.log(ns.constructor.name);
// // console.log(Virgilio.name);
// // console.log(ns instanceof Virgilio);

// // ns.constructor.name = 'bla';
// // Virgilio.name = 'foo';
// // console.log(ns.constructor.name);
// // console.log(Virgilio.name);
// // console.log(ns instanceof Virgilio);
